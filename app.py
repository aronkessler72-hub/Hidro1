"""
╔══════════════════════════════════════════════════════════════════════════════╗
║          DISEÑO HIDRÁULICO — Sistema de Autenticación con Supabase          ║
╚══════════════════════════════════════════════════════════════════════════════╝
Estructura:
  1. Importaciones y conexión Supabase
...
.auth-logo-bar .auth-eyebrow {
  font-family: 'Share Tech Mono', monospace; font-size: 0.68rem;
  color: var(--c-muted); letter-spacing: 0.25em; text-transform: uppercase; margin-bottom: 6px;
}
.auth-logo-bar .auth-title {
  font-family: 'Rajdhani', sans-serif; font-size: 2.1rem; font-weight: 700;
  color: var(--c-navy); line-height: 1.1;
}
.auth-logo-bar .auth-subtitle {
  font-family: 'Exo 2', sans-serif; font-size: 0.83rem;
  font-style: italic; color: var(--c-muted); margin-top: 4px;
}
.auth-card {
  background: var(--c-white); border: 1px solid var(--c-border);
  border-top: 4px solid var(--c-blue); border-radius: 6px;
  padding: 2rem 2.2rem 1.8rem; width: 100%; max-width: 440px;
  box-shadow: 0 8px 32px rgba(26,86,176,0.10); position: relative;
  animation: fadeSlideUp 0.5s ease both;
}
.auth-card::after {
  content: ''; position: absolute; top: 0; right: 0;
  width: 32px; height: 32px;
  border-top: 4px solid var(--c-light); border-right: 4px solid var(--c-light);
...
div[data-testid="stTabs"] button[role="tab"] {
  color: #5a7aaa !important; -webkit-text-fill-color: #5a7aaa !important;
  ...
  padding: 13px 26px !important; background: transparent !important; border-radius: 0 !important;
}
...
div[data-testid="stTabs"] button[role="tab"][aria-selected="true"] {
  color: #1a56b0 !important; -webkit-text-fill-color: #1a56b0 !important;
  border-bottom-color: #1a56b0 !important; background: rgba(26,86,176,0.07) !important;
}
...
[data-testid="stButton"] > button {
  ...
  padding: 10px 22px !important; border-radius: 2px !important; transition: all 0.2s !important;
}
...
.welcome-banner {
  ...
  border-radius: 6px; padding: 20px 30px; margin-bottom: 20px;
  display: flex; align-items: center; justify-content: space-between;
  border-bottom: 2px solid #1A56B0; animation: fadeSlideDown 0.5s ease both;
}
.welcome-banner .wb-left .wb-greeting {
  font-family: 'Share Tech Mono', monospace; font-size: 0.65rem;
  color: #6A9FDF; letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 4px;
}
.welcome-banner .wb-left .wb-name {
  font-family: 'Rajdhani', sans-serif; font-size: 1.5rem; font-weight: 700; color: #FFFFFF;
}
.welcome-banner .wb-right {
  font-family: 'Share Tech Mono', monospace; font-size: 0.62rem;
...
.dh-card::after {
  ...
  border-top: 3px solid #2a6fdb; border-right: 3px solid #2a6fdb; border-radius: 0 4px 0 0;
}
.dh-card-title {
...
.dh-sep {
  ...
  background: linear-gradient(90deg, transparent, #2a6fdb, transparent); margin: 18px 0;
}
...
def do_register(email: str, password: str) -> tuple:
    ...
    try:
        ...
        return True, "¡Cuenta creada! Revisa tu correo para confirmar tu cuenta antes de iniciar sesión."
...
def render_auth_screen():
    ...
    with col_center:
        ...
        <div class="auth-logo-bar">
          <div class="auth-eyebrow">Universidad Nacional del Altiplano · Puno</div>
          <div class="auth-title">Diseño Hidráulico</div>
        ...
        with card_container.container():
            ...
            with tab_login:
                ...
                email_in    = st.text_input("Correo electrónico",
                                            placeholder="tu@correo.com", key="login_email")
                ...
                if st.button("Ingresar al sistema", key="btn_login"):
                    if not email_in or not password_in:
                        ...
                        with st.spinner("Verificando credenciales y preparando tu espacio de trabajo..."):
                            ok, msg = do_login(email_in, password_in)
                            if ok:
                                time.sleep(0.6)
                        if ok:
                            with st.spinner("Cargando tu historial de trabajo..."):
                                cargar_historial_usuario(st.session_state["user_id"])
                                time.sleep(0.4)
            ...
            with tab_registro:
                ...
                email_reg = st.text_input("Correo electrónico",
                                          placeholder="tu@correo.com", key="reg_email")
                pass_reg  = st.text_input("Contraseña",
                                          type="password", placeholder="Mínimo 6 caracteres", key="reg_pass")
                ...
                if st.button("Crear cuenta", key="btn_register"):
                    if not email_reg or not pass_reg or not pass_reg2:
                        ...
                        with st.spinner("Creando tu cuenta..."):
                            ok, msg = do_register(email_reg, pass_reg)
                            time.sleep(0.5)
                        if ok:
                            st.success(msg)
                        else:
...
def render_welcome_banner():
    ...
    with col_banner:
        ...
        <div class="welcome-banner">
          <div class="wb-left">
            <div class="wb-greeting">Sesión activa · Espacio de trabajo</div>
            <div class="wb-name">Bienvenido, {nombre_display}</div>
          </div>
          <div class="wb-right">
            {email}<br>
...
def detectar_material_tubo(nombre_tubo: str, df_rug: pd.DataFrame) -> str:
    ...
    for mat in df_rug['Material']:
        ...
        if mat_clean and mat_clean in n_clean:
            return mat
    if 'PVC'  in nombre_tubo.upper(): return 'PVC'
    if 'HDPE' in nombre_tubo.upper(): return 'HDPE'
...
def build_excel(lista: list, datos_editados: dict) -> bytes:
    ...
    for i, r in enumerate(lista, 1):
        ws = wb.create_sheet(title=safe_sheet_name(
            f"Rep{i} {r['material']} {r['diam_nom']} {r['caudal_ls']}Ls"))
...
def build_pdf(lista: list, datos_editados: dict) -> bytes:
    ...
    doc.addPageTemplates([
        PageTemplate(id='Portrait',
                     frames=[Frame(2*cm, 1.8*cm, pw-4*cm, ph-2*cm-1.8*cm)],
    ...
    story.append(Paragraph(
        "Diseño Hidráulico — Modelamiento computacional de flujo interno en tuberías a presión", sty_sub))
        ...
        for mat in materiales:
            for ri in [1, 2]:
                for cc in range(col, col+3):
                    bs += [
                        ("BACKGROUND",(cc,ri),(cc,ri), mat_c[mat]),
...
def render_main_app():
    ...
    if n_reportes > 0:
        st.info(
            f"📂  Se cargaron **{n_reportes} reporte(s)** de tu sesión anterior. "
            "Puedes continuar en la pestaña **Hoja de Resumen**.",
            icon="📂",
        )
    else:
        st.markdown("""
        <div style="background:#f0f5fb;border:1px solid #c5d5ea;border-left:4px solid #1a56b0;
          padding:12px 18px;border-radius:3px;margin-bottom:16px;
          font-family:'Share Tech Mono',monospace;font-size:0.72rem;color:#1a3a6a;letter-spacing:1px;">
        ...
        if 'caudal_ls' not in st.session_state:
            ...
            with col_t:
                ...
                filas = [
                    ...
                    ("Rugosidad ε",         f"{eps:.7f} m"),
                    ("Relación ε/D",        f"{eD:.7f}"),
                    ("Factor fricción f",   f"{f:.5f}"),
                    ("C Hazen-Williams",    f"{C_H:.3f}"),
                ]
